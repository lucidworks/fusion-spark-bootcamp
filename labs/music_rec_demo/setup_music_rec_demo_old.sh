...
echo -e "\nCreating the music_rec_demo_old collection in Fusion"
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H "Content-type:application/json" -d '{"solrParams":{"numShards":3,"maxShardsPerNode":3}}' $FUSION_API/collections/music_rec_demo_old

echo -e "\n\nEnabling signals feature for music_rec_demo_old ..."
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H Content-type:application/json -d '{"enabled":true}' "$FUSION_API/collections/music_rec_demo_old/features/signals"
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H Content-type:application/json -d '{"enabled":true}' "$FUSION_API/collections/music_rec_demo_old/features/dynamicSchema"
...
echo -e "\n\nUpdating the schema for music_rec_demo_old to add song fields ..."
curl -u $FUSION_USER:$FUSION_PASS -XPOST -H "Content-type:application/json" "$FUSION_API/collections/music_rec_demo_old/schema/fields" \
...
curl -u $FUSION_USER:$FUSION_PASS -XPOST -H "Content-type:application/json" "$FUSION_API/collections/music_rec_demo_old_signals/schema/fields" \
...
curl -u $FUSION_USER:$FUSION_PASS -X POST -H "Content-type: application/json" "$FUSION_API/connectors/jobs/eventsim-event-crawler-old"
...
echo -e "\n\nEnabling the recommendations feature for music_rec_demo_old ... recommender spark jobs should start automatically after this step."
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H Content-type:application/json -d '{"enabled":true}' "$FUSION_API/collections/music_rec_demo_old/features/recommendations"
