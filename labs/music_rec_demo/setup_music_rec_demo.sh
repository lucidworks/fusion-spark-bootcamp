...
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H Content-type:application/json -d '{"enabled":true}' "$FUSION_API/apps/$BOOTCAMP/collections/music_rec_demo/features/dynamicSchema"

echo -e "\n\nCreating the song and event indexing pipelines"
curl -u $FUSION_USER:$FUSION_PASS -XPOST -H "Content-type:application/json" -d @eventsim-song-indexer.json "$FUSION_API/apps/$BOOTCAMP/index-pipelines"
curl -u $FUSION_USER:$FUSION_PASS -XPOST -H "Content-type:application/json" -d @eventsim-event-indexer.json "$FUSION_API/apps/$BOOTCAMP/index-pipelines"
...
curl -u $FUSION_USER:$FUSION_PASS -X POST --data-binary @eventsim-json-parser.json -H "Content-type: application/json" "$FUSION_API/apps/$BOOTCAMP/parsers"
...
curl -u $FUSION_USER:$FUSION_PASS -X POST --data-binary @eventsim-event-crawler.json -H "Content-type: application/json" "$FUSION_API/apps/$BOOTCAMP/connectors/datasources"

echo -e "\n\nStarting the crawler job to index simulated song event data from $DATA_DIR/control.data.json.zip"
curl -u $FUSION_USER:$FUSION_PASS -X POST -H "Content-type: application/json" "$FUSION_API/apps/$BOOTCAMP/connectors/jobs/eventsim-event-crawler"
...
curl -u $FUSION_USER:$FUSION_PASS -X POST --data-binary @eventsim-song-crawler.json -H "Content-type: application/json" "$FUSION_API/apps/$BOOTCAMP/connectors/datasources"

echo -e "\n\nStarting the crawler job to index simulated song data from $DATA_DIR/control.data.json.zip"
curl -u $FUSION_USER:$FUSION_PASS -X POST -H "Content-type: application/json" "$FUSION_API/apps/$BOOTCAMP/connectors/jobs/eventsim-song-crawler"
...
curl -u $FUSION_USER:$FUSION_PASS -X PUT -H Content-type:application/json -d '{"enabled":true}' "$FUSION_API/apps/$BOOTCAMP/collections/music_rec_demo/features/recommendations"
